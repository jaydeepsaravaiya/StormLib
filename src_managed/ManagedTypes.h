...
namespace ManagedStormLib {
	...
	public value class HetTable {
		IntPtr pBetIndexes;                     // Bit array of FileIndex values
	...
	public:
		...
		static UserData FromNativePointer(TMPQUserData* native) {
			UserData managed;
			if (native == nullptr)
				return managed;
			managed.dwID = native->dwID;
		...
		static UserData FromNativeStructure(TMPQUserData native) {
			UserData managed;
			managed.dwID = native.dwID;
			managed.cbUserDataSize = native.cbUserDataSize;
			managed.dwHeaderOffs = native.dwHeaderOffs;
